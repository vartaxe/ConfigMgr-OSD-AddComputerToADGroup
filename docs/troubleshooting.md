...
An already-direct-member rerun should return `0` without adding a duplicate. A successful verification uses the same directory connection and does not prove replication to every DC. All live scenarios above still require the [validation checklist](validation.md).
