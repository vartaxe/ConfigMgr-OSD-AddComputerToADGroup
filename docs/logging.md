...
**Diagnostic streams:** sanitized `WARN` and `ERROR` messages use PowerShell's warning stream (3), not its success stream (1). Their prefixes are `[WARN]` and `[ERROR]`. A discovery warning therefore cannot become a domain-controller name. CMTrace entries retain their own severity; a warning-stream diagnostic is not a substitute for checking the process exit code.
...
Raw server diagnostic text and raw exception details are not written to the dedicated log or console. Exception unwrapping preserves the first structured LDAP code or directory response, even when it has an inner cause. That code determines retry classification; server-provided text does not override it. Investigate additional details through approved private directory diagnostics rather than enabling raw-exception logging in the script.
...
Include both logs in your [environment validation](validation.md#required-live-tests), especially invalid-credential and failure paths.
