...
- Hidden variables are not a security boundary against a local administrator or Local System. Credentials remain accessible to the Task Sequence process. The script disposes its own secure-password buffer; that does not erase every managed credential copy or replace clearing the Task Sequence variables.
- Review sanitized copies of both `AddComputerToADGroup.log` and `smsts.log` before sharing. Directory names and deployment details can still be sensitive. Include both logs in the [environment-validation checklist](docs/validation.md#required-live-tests).
